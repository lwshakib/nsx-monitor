        ...
        <div className="flex flex-col items-center justify-center fade-up relative z-10 px-6 md:px-10">
          <div className="relative w-full overflow-hidden mb-24 flex items-center opacity-20 group-hover:opacity-40 transition-opacity duration-1000">
            <div className="flex whitespace-nowrap animate-[marquee_25s_linear_infinite]">
              <h2 className="text-[10vw] font-bold uppercase tracking-tighter text-aura-text mx-8">DEPLOYMENTS</h2>
              <h2 className="text-[10vw] font-bold uppercase tracking-tighter text-transparent mx-8 stroke-aura-text" style={{ WebkitTextStroke: '1px var(--aura-text)' }}>DEPLOYMENTS</h2>
              <h2 className="text-[10vw] font-bold uppercase tracking-tighter text-aura-text mx-8">DEPLOYMENTS</h2>
              <h2 className="text-[10vw] font-bold uppercase tracking-tighter text-transparent mx-8 stroke-aura-text" style={{ WebkitTextStroke: '1px var(--aura-text)' }}>DEPLOYMENTS</h2>
            </div>
          </div>
          ...
          {deployments.map((deployment, idx) => (
            <a key={deployment.title} href="#" className={`group/proj block relative rounded-[2rem] overflow-hidden aspect-[4/3] border border-aura-border hover:border-aura-border-bright transition-all duration-500 shadow-2xl bg-aura-bg-alt p-2 ${idx === 1 ? 'md:mt-24' : ''}`}>
              <div className="absolute top-6 right-6 z-20 bg-brand-surface/80 backdrop-blur-md border border-aura-border text-aura-text font-mono text-[10px] px-3 py-1 rounded-full opacity-0 group-hover/proj:opacity-100 translate-y-2 group-hover/proj:translate-y-0 transition-all duration-300 flex items-center gap-2">
              ...
              <div className="relative w-full h-full rounded-[1.5rem] overflow-hidden bg-brand-surface">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-brand-bg via-brand-bg/40 to-transparent flex flex-col justify-end p-8">
                  <div className="translate-y-4 group-hover/proj:translate-y-0 transition-transform duration-500 text-left">
                    <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-aura-text-muted mb-2 block opacity-0 group-hover/proj:opacity-100 transition-opacity duration-500">
                      {deployment.category}
                    </span>
                    <h3 className="text-3xl font-semibold tracking-tight text-aura-text">
                      {deployment.title}
                    </h3>
                  </div>
                </div>
