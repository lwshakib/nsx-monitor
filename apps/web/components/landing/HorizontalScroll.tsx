  ...
  return (
    <section ref={sectionRef} className="ui-section h-screen relative" id="engine">
      <div className="sticky h-screen top-0 items-start ui-container border-y border-ui-border flex flex-col justify-center overflow-hidden">
        ...
        <div className="absolute -bottom-[7.5px] -left-[7.5px] w-3.5 h-3.5 text-ui-border-bright z-50">
          <Icon icon="solar:add-linear" />
        </div>
        <div className="absolute -bottom-[7.5px] -right-[7.5px] w-3.5 h-3.5 text-ui-border-bright z-50">
          <Icon icon="solar:add-linear" />
        </div>

        <div className="absolute top-12 left-6 md:left-10 z-20 flex items-center gap-4">
          <div className="w-12 h-[1px] bg-ui-border-bright"></div>
          <span className="text-[10px] font-mono tracking-[0.2em] font-bold text-ui-text uppercase">
            Engine Mechanics
          </span>
        </div>

        <div className="absolute bottom-12 left-6 md:left-10 z-20 w-48 h-1 bg-ui-border/20 rounded-full overflow-hidden">
          <div ref={progressRef} className="h-full bg-ui-text w-0 transition-all duration-100 ease-out" id="h-scroll-progress"></div>
        </div>
        ...
        <div ref={trackRef} className="flex will-change-transform pr-[20vw] pl-[10vw] gap-8 items-center" style={{ width: 'max-content' }} id="h-scroll-track">
          {cards.map((card, idx) => (
            <div key={card.id} className="schema-card schema-card-enhanced w-[85vw] md:w-[450px] aspect-square rounded-[2rem] p-10 flex flex-col relative overflow-hidden flex-shrink-0 group transition-all duration-500 shadow-2xl bg-ui-bg border border-ui-border">
              ...
              <span className="text-6xl font-light text-ui-text opacity-10 absolute top-6 right-8 tracking-tighter font-mono group-hover:opacity-30 transition-all duration-500">
                {card.id}
              </span>

              <div className="w-12 h-12 rounded-xl bg-brand-surface border border-ui-border flex items-center justify-center mb-auto mt-2 relative z-10 shadow-lg group-hover:shadow-[0_0_20px_var(--accent-glow)] transition-all">
                {/* SVG icons simplified for the sake of the card structure */}
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-ui-text">
              ...
              <div className="relative z-10 mt-auto text-left">
                <h3 className="text-2xl font-semibold mb-4 tracking-tight text-ui-text group-hover:translate-x-1 transition-transform">
                  {card.title}
                </h3>
